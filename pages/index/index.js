...
Page({
  ...
  onLoad: function (options) {
    ...
    setInterval(() => {
      ...
      const updated = this.data.agreements.map(a => {
        if (a.status === "Active - Awaiting President Approval" && a.releaseDate) {
  ...
  closeDialog: function () {
    this.setData({ isDialogOpen: false, newTitle: '', newAmount: '', newVendorAddress: '', newTerms: '' })
  },
  ...
  closeVendorDialog: function () {
    this.setData({ isVendorDialogOpen: false, vendorProofText: '', vendorProofUrl: '', activeAgreementId: '' });
  },
  ...
  handleCreateAgreement: function () {
    const { newTitle, newAmount, newVendorAddress, newTerms, agreements } = this.data;
    if (!newTitle || !newAmount || !newVendorAddress.trim() || !newTerms.trim()) {
    ...
    this.setData({ agreements: [newAgreement, ...agreements], newTitle: '', newAmount: '', newVendorAddress: '', newTerms: '', isDialogOpen: false });
  },
  ...
  handleMarkComplete: function () {
    const { activeAgreementId: id, vendorProofText, vendorProofUrl, agreements, walletAddress } = this.data;
    ...
    let updated = agreements.map(a => a.id === id ? { ...a, status: "Verifying Completion", vendorProofText, vendorProofUrl } : a);
  ...
  executeOracleCall: function (id, vendorProof) {
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        const updated = this.data.agreements.map(a =>
          a.id === id ? { ...a, status: "Active - Awaiting President Approval", aiAnalysis: reply, releaseDate } : a
        );
        this.setData({ agreements: updated });
      },
      fail: (err) => {
